...
const AddSimulation = () => {
  ...
  // Fetch simulation data using the simulationId and companyId
  ...
  // Listen for task count updates and propagate changes
  useEffect(() => {
    const handleTaskCountUpdate = (event: CustomEvent<TaskCounts>) => {
      if (event.detail) {
        setTaskCounts(event.detail);
        window.dispatchEvent(
          new CustomEvent("taskCountsUpdated", { detail: event.detail }),
        );
      }
    };
    ...
    <MainLayout>
      <div className="bg-[#F8F9FA] flex flex-wrap items-start gap-[35px] overflow-hidden p-8">
        <div className="grow shrink-0 basis-0 w-fit max-md:max-w-full">
          <Header />
          <MainContent simulationData={simulationData} />
        </div>
      </div>
    </MainLayout>
  );
...
const MainContent = ({ simulationData }: MainContentProps) => (
